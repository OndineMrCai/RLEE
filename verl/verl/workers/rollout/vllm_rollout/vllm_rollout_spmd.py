...
class vLLMRollout(BaseRollout):
    ...
    def generate_branch(self, gen_batch: DataProto, exploration_token: List[torch.tensor], **kwargs) -> DataProto:
        ...
        exploration_position_mask = _repeat_interleave(exploration_position_mask, kwargs['n'])

        if not new_prompts:
            return gen_batch
        
        forbidden_processor = FirstTokenForbiddenProcessor(forbidden_token_ids=exploration_token)
        vllm_inputs = [{'prompt_token_ids': prompt} for prompt in new_prompts]
