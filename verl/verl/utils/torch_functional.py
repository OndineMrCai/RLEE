...
def pad_2d_list_to_length_left(prompts, pad_token_id, max_length=None):
    """
    pad a 2D list (e.g. prompts) to a 2D tensor.
    """
    prompt_length = max(len(sub_prompt_list) for sub_prompt_list in prompts)
    if max_length is not None and max_length > prompt_length:
        target_length = max_length
    else:
        target_length = prompt_length
    padded_response = [(pad_token_id,) * (target_length - len(sub_prompt_list)) + tuple(sub_prompt_list) for sub_prompt_list in prompts]
    tensor = torch.tensor(padded_response)
    return tensor
